...
if __name__ == '__main__':
    ...
    train_col = "originalTitle"
    duckDB_col = "original_title"
    ...
    train_original_title = []
    for i in range(len(full_train_df)):
        curr = full_train_df.iloc[i]['originalTitle']
        if isinstance(curr, str):
            train_original_title.append([tconst_train[i], unidecode.unidecode(curr)])
        else:
            train_original_title.append([tconst_train[i], ''])
    
    test_original_title = []
    for i in range(len(full_test_df)):
        curr = full_test_df.iloc[i]['originalTitle']
        if isinstance(curr, str):
            test_original_title.append([tconst_test[i], unidecode.unidecode(curr)])
        else:
            test_original_title.append([tconst_test[i], ''])

    validation_original_title = []
    for i in range(len(full_validation_df)):
        curr = full_validation_df.iloc[i]['originalTitle']
        if isinstance(curr, str):
            validation_original_title.append([tconst_validation[i], unidecode.unidecode(curr)])
        else:
            validation_original_title.append([tconst_validation[i], ''])
