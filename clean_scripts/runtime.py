...
if __name__ == '__main__':
    ...
    train_col = "runtimeMinutes"
    duckDB_col = "runtime_minutes"
    ...
    for i in range(len(full_train_df)):
        ...
        if curr == "\\N":
            train_runtime_minutes.append([tconst_train[i], int()])
        else:
            train_runtime_minutes.append([tconst_train[i], int(curr)])
    
    test_runtime_minutes = []
    for i in range(len(full_test_df)):
        curr = full_test_df.iloc[i]['runtimeMinutes']
        if curr == "\\N":
            test_runtime_minutes.append([tconst_test[i], int()])
        else:
            test_runtime_minutes.append([tconst_test[i], int(curr)])
    
    validation_runtime_minutes = []
    for i in range(len(full_validation_df)):
        curr = full_validation_df.iloc[i]['runtimeMinutes']
        if curr == "\\N":
            validation_runtime_minutes.append([tconst_validation[i], int()])
        else:
            validation_runtime_minutes.append([tconst_validation[i], int(curr)])
