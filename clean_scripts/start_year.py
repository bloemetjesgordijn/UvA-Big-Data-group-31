...
if __name__ == '__main__':
    ...
    for i in range(len(full_train_df)):
        ...
        if curr == "\\N":
            train_start_year.append([tconst_train[i], int()])
        else:
            train_start_year.append([tconst_train[i], int(curr)])

    test_start_year = []
    for i in range(len(full_test_df)):
        curr = full_test_df.iloc[i]['startYear']
        if curr == "\\N":
            test_start_year.append([tconst_test[i], int()])
        else:
            test_start_year.append([tconst_test[i], int(curr)])

    validation_start_year = []
    for i in range(len(full_validation_df)):
        curr = full_validation_df.iloc[i]['startYear']
        if curr == "\\N":
            validation_start_year.append([tconst_validation[i], int()])
        else:
            validation_start_year.append([tconst_validation[i], int(curr)])
