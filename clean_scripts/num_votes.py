...
if __name__ == '__main__':
    ...
    for i in range(len(full_train_df)):
        ...
        if curr == 'nan' or curr == "NaN" or math.isnan(curr):
            train_num_votes.append([tconst_train[i], int()])
        else:
            train_num_votes.append([tconst_train[i], int(curr)])
    
    test_num_votes = []
    for i in range(len(full_test_df)):
        curr = full_test_df.iloc[i]['numVotes']
        if curr == 'nan' or curr == "NaN" or math.isnan(curr):
            test_num_votes.append([tconst_test[i], int()])
        else:
            test_num_votes.append([tconst_test[i], int(curr)])
    
    validation_num_votes = []
    for i in range(len(full_validation_df)):
        curr = full_validation_df.iloc[i]['numVotes']
        if curr == 'nan' or curr == "NaN" or math.isnan(curr):
            validation_num_votes.append([tconst_validation[i], int()])
        else:
            validation_num_votes.append([tconst_validation[i], int(curr)])
